...
class Buyer(db_conn.DBConn):
    ...
    def new_order(
        ...
        try:
            ...
            # print(user_id, store_id, id_and_count, uid)
    ...
    def payment(self, user_id: str, password: str, order_id: str) -> (int, str):
        try:
            result = self.db.orders.find_one({"order_id": order_id, "status": 0})
            if result is None:
                return error.error_invalid_order_id(order_id)
            ...
            result = self.db.user_col.find_one({"user_id": buyer_id})
            if result is None:
            ...
            result = self.db.store_col.find_one({"store_id": store_id})
            ...
            # 删除没有支付的订单记录
            result = self.db.orders.delete_one({"order_id": order_id, "status": 0})
